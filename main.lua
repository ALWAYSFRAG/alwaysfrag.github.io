...
if self == Camera and key == "CFrame" then
    local thirdPersonEnabled = menu.values[3].world.self["third person"].Toggle
    local thirdPersonActive = menu.values[3].world.self["$third person"].Active
    local thirdPersonDistance = menu.values[3].world.self.distance.Slider

    if thirdPersonEnabled and thirdPersonActive then
        value = value + Camera.CFrame.LookVector * -thirdPersonDistance
    end
end
